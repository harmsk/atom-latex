...
export default class Composer extends Disposable {
  ...
  async build (shouldRebuild) {
    ...
    const jobs = jobnames.map(jobname => this.buildJob(builder, rootFilePath, jobname, shouldRebuild, false))
  ...
  async buildJob (builder, rootFilePath, jobname, shouldRebuild, shouldClean) {
    try {
      const statusCode = await builder.run(rootFilePath, jobname, shouldRebuild, shouldClean)
  ...
  async clean () {
    ...
    if (Object.getPrototypeOf(builder).constructor.name === 'MakeBuilder') {
      const jobs = jobnames.map(jobname => this.buildJob(builder, rootFilePath, jobname, false, true))
